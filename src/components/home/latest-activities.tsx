...
const LatestActivities = () => {
  ...
  const { data: deals, isLoading: isLoadingDeals } = useList({
    ...
    filters: [{ field: 'id', operator: 'in', value: dealIds }], // Fixed filter value
    meta: {
      gqlQuery: DASHBOARD_LATEST_ACTIVITIES_DEALS_QUERY // Corrected to deals query
    }
  });
        ...
        <List
          ...
          renderItem={item => {
            ...
            return (
              <List.Item>
                <List.Item.Meta
                  title={dayjs(item?.createdAt).format('MMM DD, YYYY - HH:mm')} // Ensure valid date before formatting

                  avatar={<CustomAvatar
                    shape="square"
                    size={48}
                    src={deal?.company?.avatarUrl} // Added null check for deal and company
                    name={deal?.company?.name} // Added null check for deal and company
                  />}
                  description={
                    <Space size={4}>
